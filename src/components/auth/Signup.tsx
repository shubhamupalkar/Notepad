...
export const Signup: React.FC<SignupProps> = ({ onToggleAuthMode }) => {
    ...
    <div className="w-full min-h-screen flex items-center justify-center p-6 select-text">
      <div className="w-full max-w-md bg-white/70 backdrop-blur-md border border-[#986ddb]/15 rounded-[32px] p-8 md:p-10 shadow-[0_16px_48px_rgba(152,109,219,0.12)]">
        ...
        <div className="text-center mb-8 select-none">
          ...
          <p className="text-xs text-[#221733]/50 mt-1.5 font-medium">
            Start writing with a secure, personal cloud space.
          </p>
        </div>

        {/* Success Screen */}
        {success ? (
          <div className="text-center py-6 select-none space-y-4">
            <div className="w-12 h-12 rounded-full bg-emerald-50 text-emerald-600 flex items-center justify-center mx-auto border border-emerald-100 shadow-sm">
              <CheckCircle size={24} />
            </div>
            <h3 className="text-base font-bold text-[#221733]">Registration Successful!</h3>
            <p className="text-xs text-[#221733]/65 max-w-xs mx-auto leading-relaxed">
              We've created your account. If email confirmation is enabled on the server, please check your inbox to activate your account. Otherwise, you can log in immediately.
            </p>
            <button
              onClick={onToggleAuthMode}
              className="px-6 py-2.5 rounded-2xl bg-[#986ddb] hover:bg-[#8557c7] text-white text-xs font-bold shadow-sm transition-all cursor-pointer"
            >
              Go to Sign In
            </button>
          </div>
